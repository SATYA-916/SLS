...
### Domain
- **Domain:** `slsnexus.com` (registered with the domain registrar; DNS is delegated to Cloudflare)

### DNS hosting
- **DNS/domain management is handled through Cloudflare.** The domain's authoritative nameservers are Cloudflare's:
  - `sureena.ns.cloudflare.com`
  - `grant.ns.cloudflare.com`

### Verified DNS records (publicly resolvable, verified 2026-08-11)

| Type | Name | Value | Purpose |
|---|---|---|---|
| A | `slsnexus.com` (apex/root) | `64.29.17.65`, `216.198.79.65` | Cloudflare-proxied apex pointing to Vercel's edge (via Cloudflare proxy). These are Cloudflare anycast IPs fronting the Vercel site. |
| CNAME | `www.slsnexus.com` | `813ce86a24bb245a.vercel-dns-017.com` | Points `www` to Vercel's official DNS target. |
| TXT | `slsnexus.com` | `google-site-verification=JLRxZ00Q9UJ6SYnSkD_T25H7RPPbg61BP7Eh9-wpZm4` | Verifies the domain in Google Search Console. |
| NS | `slsnexus.com` | `sureena.ns.cloudflare.com`, `grant.ns.cloudflare.com` | Delegates DNS to Cloudflare. |

> **Note:** The live TXT record contains a valid Google site-verification token. The `google-site-verification` **meta tag in `frontend/index.html` is currently empty** — it can either be filled with the same token or removed, since the DNS TXT method already verifies the property. Do **not** publish the token value in this guide; it is already public DNS data.

### Relationship between Cloudflare, Vercel, and the website
```
User → slsnexus.com → Cloudflare (DNS + proxy/CDN/security layer)
                            ↓ (proxied records)
                      ...
                      MongoDB Atlas + Brevo
```
- **Cloudflare** is the DNS provider and sits in front of the site (orange-cloud proxied), providing CDN caching, SSL, and DDoS protection. Changing records is done in the Cloudflare dashboard → DNS.
...
> **Do not** expose Cloudflare account passwords or API tokens. If DNS changes are needed, the Cloudflare dashboard is accessed by the account owner.
